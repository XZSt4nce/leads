...
<html>
	...
	<body>
		<!-- Заголовок первого уровня, выровненный посередине -->
		...
		<input id="start_date" name="start_date" type="date" 
			...
			<!-- По умолчанию начальной датой является предыдущий день -->
		<label for="end_date" style="display:inline;">по</div>
		<input id="end_date" name="end_date" type="date" 
			value="<?php echo date("Y-m-d"); ?>">
			<!-- По умолчанию конечной датой является текущий день -->
		...
		<table class="table table-sm table-striped table-responsive">
			...
			$query = "
				...
				WHERE table_name = 'efficiency_eval' AND column_name LIKE 'object_%';";
			...
			$headers = array("#", "Фамилия", "Имя", "Отчество", "ID сотрудника", "Лидов обработано", "Лидов подписало договор",
				"Некачественных лидов", "Процент завершения");
			$objects = array();
			...
			// Подготовка запроса
			$efficiency = $link->prepare(sprintf("
				SELECT %s 
				FROM 
					efficiency_eval 
					JOIN b_user ON efficiency_eval.user_id = b_user.id 
				WHERE period = ?;", 
				implode(', ', $fields)));

			function date_out ($headers, $date) {
				return "<tr><th class='table-dark' style='padding-left: 30px' colspan=" . 
					count($headers) . ">" . $date . "</th></tr>";
			}
			...
			if (isset($_GET['done'])) {
				...
				// Если не заполнена только начальная дата
					...
					// Присваивание строкового GET-параметра плейсхолдеру подготовленного запроса
					...
					// Вывод даты
					echo date_out($headers, $_GET['end_date']);

					// Вывод данных на конкретную дату
					while($row = $result->fetch_assoc()) {
						echo "<tr> <th scope='row' class='table-primary'>". $num . "</th>"; // Вывод номера строки
				...
				// Если не заполнена только конечная дата
					...
					echo date_out($headers, $_GET['start_date']);
					while($row = $result->fetch_assoc()) {
				...
				// В остальных случаях:
					...
					// Определение конечной даты и даты-итератора
					...
					while ($timer <= $end) {
						...
						// Для каждой даты начинать счёт с единицы
						$num = 1;
					}
				}
